...
def modify_attempt(source: Agent):
    ...
    my_shuffle(source.rejected, k=194)
    source.rejected.sort(reverse=True)
...
print([a.score for a in agents][:10], f'\ntime elapsed: {(ts() - st) * 1000:.3f}ms')